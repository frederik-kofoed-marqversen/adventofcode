...
pub fn run(use_test_input: bool) {
    ...
    // PART 1
    let mirror_positions: Vec<usize> = patterns
        .iter()
        .map(|pattern| match find_mirror_rows(pattern, false) {
            Some(i) => 100 * i,
    ...
    let mirror_positions: Vec<usize> = patterns
        .iter()
        .map(|pattern| match find_mirror_rows(pattern, true) {
            Some(i) => 100 * i,
...
fn find_mirror_rows(pattern: &Pattern, smutch: bool) -> Option<usize> {
    ...
    for i in 1..pattern.len() {
        ...
        for d in 1..=distance_to_edge {
            mismatches += zip(&pattern[i - d], &pattern[i + d - 1])
                .filter(|(a, b)| a != b)
                .count();
        }
