...
pub fn run(use_test_input: bool) {
    let input = super::read_input(19, use_test_input);
    ...
    let ratings = accepted_parts
        .iter()
        .map(|part| part.x + part.m + part.a + part.s);
    println!("Result part 1: {}", ratings.sum::<u64>());

    // PART 2
    let input_space = Region4 {
        data: [Interval {
            lower: 1,
            upper: 4_000,
        }; 4],
            ...
            match rule.split_once(':') {
                Some((comparison, new_destination)) => {
                    ...
                    let (positive, negative) = match comparator {
                        '>' => (Interval::new(bound + 1, u64::MAX), Interval::new(0, bound)),
                        '<' => (Interval::new(0, bound - 1), Interval::new(bound, u64::MAX)),
                        _ => {
...
impl Region4 {
    fn volume(&self) -> u64 {
        self.data
            .iter()
            .fold(1, |prod, interval| prod * interval.volume())
    }
}
