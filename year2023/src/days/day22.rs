...
pub fn run(use_test_input: bool) {
    let input = super::read_input(22, use_test_input);
    let mut snapshot = parse_input(&input);
    // dbg!(&snapshot);
    ...
    for brick in &snapshot {
        ...
        for (i, other) in brick_pile.iter().enumerate() {
            if brick[0].intersection(&other[0]).volume() > 0
                && brick[1].intersection(&other[1]).volume() > 0
            {
                hits.push(i);
            }
        }
        ...
        let final_height = hits
            .iter()
            .map(|&i| brick_pile[i][2].upper + 1)
            .max()
            .unwrap_or(1);

        // Current brick is supported by bricks one below its final height
        let support: Vec<usize> = hits
            .into_iter()
            .filter(|&i| brick_pile[i][2].upper == final_height - 1)
            .collect();
        ...
        brick_pile.push([
            ...
            Interval {
                lower: final_height,
                upper: final_height + brick[2].volume() - 1,
            },
        ]);
    }
    ...
    for i in 0..brick_pile.len() {
        ...
        for j in i + 1..brick_pile.len() {
            // check if j will fall
            if supported_by[j].len() == 0 {
                continue;
            } // brick lies on ground and will not fall
    ...
    println!(
        "Result part 2: {}",
        will_fall_if_disintegrated
            .iter()
            .fold(0, |sum, list| sum + list.len())
    );
}
...
fn parse_input(input: &str) -> Vec<Brick> {
    ...
    for line in input.trim().split('\n') {
        let endpoints: Vec<Vec<u32>> = line
            .split('~')
            .map(|coords| coords.split(',').map(|n| n.parse().unwrap()).collect())
            .collect();
        let brick = [
