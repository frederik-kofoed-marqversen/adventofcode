...
pub fn run(use_test_input: bool) {
    ...
    if modules["rx"].input_modules.len() != 1 {
        panic!("Not exactly one input")
    }
    let primary_conjunction = modules["rx"].input_modules[0].clone();
    if modules[&primary_conjunction].module_type != "conjunction" {
    ...
    println!(
        "Result part 2: {}",
        periods.iter().fold(1, |res, p| lcm(res, *p))
    );
}
...
impl Module {
    fn from_prefix(prefix: char) -> Self {
        ...
        match prefix {
            // luckily bools default to false
            '%' => {
