...
use rust_aoc_lib::graph::algorithms::{a_star, Traversible};

pub fn run(use_test_input: bool) {
    let input = super::read_input(17, use_test_input);
    ...
    let manhattan = |state: &State| {
        (state.position.0.abs_diff(end_pos.0) + state.position.1.abs_diff(end_pos.1)) as u64
    };
    ...
    println!(
        "Result part 1: {}",
        a_star(&map, &start, &end_condition, Some(&manhattan))
            .unwrap()
            .0
    );
...
impl Traversible<State, u64> for Map {
    fn connections(&self, state: &State) -> HashMap<State, u64> {
        ...
        for new_direction in ["north", "south", "east", "west"] {
            let new_step_num = if new_direction == direction {
                step_num + 1
            } else {
                1
            };

            if part2 && step_num < 4 && new_direction != direction && direction != "None" {
                continue;
            }
            if (!part2 && new_step_num > 3) || (part2 && new_step_num > 10) {
                continue;
            }
            ...
            let new_position = match new_direction {
                "north" => {
                    if position.0 == 0 || direction == "south" {
                        continue;
                    }
                    (position.0 - 1, position.1)
                }
                "south" => {
                    if position.0 == self.n - 1 || direction == "north" {
                        continue;
                    }
                    (position.0 + 1, position.1)
                }
                "west" => {
                    if position.1 == 0 || direction == "east" {
                        continue;
                    }
                    (position.0, position.1 - 1)
                }
                "east" => {
                    if position.1 == self.n - 1 || direction == "west" {
                        continue;
                    }
                    (position.0, position.1 + 1)
                }
