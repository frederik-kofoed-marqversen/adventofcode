...
fn main() {
    ...
    let mut num_steps = 0;
    while numbers.len() < 3 { // Need at least three points to determine second order polynomial
        num_steps += 1;
    ...
    let coeffs = [numbers[0], numbers[1] - numbers[0], numbers[2] - 2*numbers[1] + numbers[0]];
...
fn parse_file(filepath: &str) -> Result<DataStruct, std::io::Error> {
    // We pad the map with stones to avoid boundary cases
    Ok(
